...
\begin{document}
	...
	\begin{abstract}
		\noindent\textbf{Background.} Large language models (LLMs) are being applied across mental health
		research at pace, but the methods used to build, adapt and evaluate them have not been synthesised
		systematically. Seven reviews published between 2024 and 2026 organise the field around application
		areas rather than methodology, and every one closed its search before the majority of the current
		literature appeared.
		
		\noindent\textbf{Objective.} To synthesise, compare and critically appraise the research
		methodologies used in LLM-based mental health studies, and to characterise the field's evidential
		standards rather than its reported performance.
		
		\noindent\textbf{Methods.} Methodology-only extraction was performed across 66 unique studies
		(68 retrieved documents, two preprint/published pairs merged) using a uniform 24-field instrument.
		...
		\noindent\textbf{Conclusions.} The field's methodological weaknesses are concentrated in its earliest
		work and are narrowing. Present constraints on interpretation are the scarcity of reported agreement
		statistics, the gap between nominal and substantive risk evaluation, and the absence of powered
		clinical trials.
	\end{abstract}
	...
	Mental health services worldwide operate under sustained workforce shortage, and large language
	models (LLMs) have been proposed as a partial response, in screening, triage, psychoeducation,
	counselling support and clinical documentation. Research output has grown quickly: of the 66 studies
	synthesised here, 45 (68\%) were published in 2025 or 2026.
	
	Growth of this kind raises a question distinct from whether these systems perform well, namely
	...
	Seven systematic or scoping reviews of LLMs in mental health appeared between 2024 and
	2026\cite{Jin2025LLMMentalHealth,Hua2025LLMMentalHealthScoping,fpsyg.2025.1715306,Hua2025,WANG2025,Kolding2025,GUO2024}.
	Three properties limit what they can currently support. None performs quantitative synthesis, so no
	pooled effect estimate exists for any LLM mental health application. Formal quality appraisal is
	uncommon, applied in two of the seven. Most importantly, every search window closes before the
	...
	application areas, describing what LLMs are used for. None is organised around methodology as
	such: how corpora are constructed, how models are adapted, how evaluation is designed and how risk
	is assessed. That is the organising principle adopted here.
	
	\subsection{Objectives and research questions}
	
	This review synthesises the methodologies used across LLM-based mental health research and appraises
	their evidential adequacy. Four questions structure the synthesis:
	
	\begin{enumerate}[itemsep=1pt,topsep=3pt]
		\item What data foundations support this literature, and how is ground truth established?
		(Section 4)
	...
	% TODO: state registration (PROSPERO / OSF identifier) or declare explicitly that the review
	% was not registered. Journals increasingly require one or the other.
	This review was reported with reference to the PRISMA 2020 statement. \textbf{[TODO: registration
		statement --- insert PROSPERO or OSF identifier, or an explicit declaration that the review was not
		prospectively registered.]}
	
	\subsection{Information sources and search strategy}
	
	\textbf{[TODO: databases searched, platform/interface, and the date each search was executed.]}
	
	\textbf{[TODO: full search strings per database. Reproduce verbatim, including Boolean operators,
		truncation and field tags. Place the complete strategy in Supplementary Material S1.]}
	
	\textbf{[TODO: any supplementary search methods --- backward or forward citation chaining, preprint
		servers, hand-searching.]}
	
	\subsection{Eligibility criteria}
	
	\textbf{[TODO: inclusion criteria --- study designs admitted, population or task scope, model scope
		(e.g. a parameter or architecture threshold), publication types, language, date range.]}
	
	\textbf{[TODO: exclusion criteria, with the reason each is applied.]}
	
	\subsection{Study selection}
	
	\textbf{[TODO: number of reviewers screening titles/abstracts and full texts; whether screening was
		independent and in duplicate; how disagreements were resolved; any screening software used;
		inter-screener agreement if measured.]}
	
	Screening yielded 68 documents for extraction. Two studies were retrieved in both preprint and
	published form; each pair was merged into a single record, giving 66 unique studies. The PRISMA
	flow of records through identification, screening, eligibility and inclusion is shown in
	Figure~\ref{fig:prisma}.
	
	\begin{figure}[htbp]
		\centering
		\fbox{\begin{minipage}[c][7cm][c]{0.9\textwidth}
				\centering
				\textbf{[PRISMA 2020 flow diagram --- to be inserted]}\\[6pt]
				\footnotesize
				Records identified ($n =$ \underline{\hspace{1.5cm}}) $\rightarrow$
				duplicates removed ($n =$ \underline{\hspace{1.5cm}}) $\rightarrow$
				records screened ($n =$ \underline{\hspace{1.5cm}}) $\rightarrow$
				records excluded ($n =$ \underline{\hspace{1.5cm}}) $\rightarrow$
				full texts assessed ($n =$ \underline{\hspace{1.5cm}}) $\rightarrow$
				full texts excluded with reasons ($n =$ \underline{\hspace{1.5cm}}) $\rightarrow$
				documents included ($n = 68$) $\rightarrow$
				unique studies after merging preprint/published pairs ($n = 66$).
		\end{minipage}}
		\caption{PRISMA 2020 flow of records through the review.}
		\label{fig:prisma}
	\end{figure}
	
	\subsection{Data extraction}
	
	Extraction was methodology-only. Where prior reviews chart what models achieved, this review charts
	how the achievement was established, and reported results were deliberately excluded from the
	extraction instrument.
	
	Every study was extracted across the same twenty-four fields: paper identifier, title, year,
	research objective, mental health task, methodology category, dataset(s), data type, LLM(s) used,
	baseline models, prompting strategy, fine-tuning or retrieval or agentic framework, experimental
	design, evaluation protocol, evaluation metrics, human evaluation, statistical analysis,
	hallucination evaluation method, bias evaluation method, safety evaluation method, implementation
	details, strengths, limitations and novel methodological contribution.
	
	Four of these fields (hallucination, bias, safety and human evaluation) are not extracted
	systematically by any prior review, and they support the corpus-wide statements made in Sections 8
	and 9. Absence was recorded as data: ``Not Reported'' was distinguished throughout from ``not
	applicable'', so that gaps in the primary literature are recoverable as findings rather than
	appearing as blanks.
	
	\textbf{[TODO: number of extractors; whether extraction was performed in duplicate or verified by a
		second reviewer; how extraction disagreements were resolved.]}
	...
	substantive risk evaluation. Safety appears in 61 of 66 studies and bias in 58, figures that would
	support a favourable account of the field's attention to risk. Decomposition gives a different
	...
	5 apply a recognisable method. Any synthesis reporting the headline percentages without this
	decomposition would mislead.
	
	The second concerns human judgement. It serves as ground truth in most of this literature, with 33
	of 59 studies using licensed clinical professionals, yet 45 of those 59 report no agreement
	...
	inverse. Studies presenting full pipelines, agentic orchestration or deployed interfaces report the
	least evaluation, while narrower studies report the most. Section 9 shows this is a corpus-level
	pattern rather than an artefact of selective reading: adaptation studies report inferential
	statistics in 21\% of cases and reliability coefficients in none, against 83\% and 50\% for
	benchmark studies.
	
	\subsection{Comparison with prior reviews}
	
	Existing syntheses reach broadly compatible conclusions about capability but cannot address
	methodological adequacy, for two reasons established in Section 3. Their search windows close
	before 68\% of the present corpus was published, so they characterise a methodologically weaker
	literature than the one now current; and none extracts risk methodology systematically, so the
	coverage-versus-measurement distinction developed in Section 8 is not available to them.
	...
	Two design choices would yield disproportionate returns. Reporting decoding parameters costs
	nothing and would remove a substantial source of unquantified variance, since identical prompts
	yield different answers across runs and repetition is inconsistently applied. Reporting an agreement
	statistic wherever human raters are used would allow the field's primary ground truth to be
	...
	exploratory trial constitutes the entire prospective evidence base, so every efficacy claim in this
	literature depends on proxy measures, whether benchmark accuracy, expert appropriateness ratings or
	...
	with improved accuracy in the reviewed studies, so a model marketed as clinically specialised should
	not be assumed to outperform a general-purpose counterpart without matched-pair evidence. And
	...
	evaluation is required because therapeutic relationships are longitudinal and the evaluation
	apparatus largely is not. Adequately powered trials measuring patient outcomes are needed to move
	...
	more often than measured. Finally, evaluation resources outside English and outside Western
	platforms remain scarce, and the cultural mismatch reported by users cannot be investigated with
	the corpora currently available.
	
	\subsection{Limitations of this review}
	
	Five limitations qualify the findings. No quantitative pooling was performed, so no effect estimate
	...
	fields derive from optical character recognition. \textbf{[TODO: add any limitation arising from
		single-reviewer screening or extraction once Section 2.4 and 2.5 are completed.]}
	
	\section{Conclusion}
	
	Across 66 studies, the methods used to establish that large language models are useful in mental
	health are less developed than the methods used to build them. Inferential statistics support fewer
	...
	These weaknesses are concentrated in the field's earliest work. Inferential reporting rises from
	zero in 2023 to 67\% in 2026 and reliability reporting from zero to 44\% across the same period,
	which suggests that what this review documents is the early stage of a literature rather than a
	fixed characteristic of it. The components of a stronger design already exist within the corpus.
	Assembling them into single studies, and reporting them fully, is the work that remains.
	
	\section*{Declarations}
	
	\noindent\textbf{Funding.} \textbf{[TODO]}
	
	\noindent\textbf{Conflicts of interest.} \textbf{[TODO]}
	
	\noindent\textbf{Author contributions.} \textbf{[TODO --- CRediT taxonomy recommended.]}
	
	\noindent\textbf{Data availability.} The complete 24-field extraction matrix for all 66 included
	studies is provided as Supplementary Material. \textbf{[TODO: add repository DOI.]}
	
	\noindent\textbf{Supplementary material.} S1: full search strategy per database. S2: PRISMA 2020
	checklist. S3: study characteristics table. S4: complete extraction matrix.
